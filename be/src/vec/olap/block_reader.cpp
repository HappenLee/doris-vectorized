...
OLAPStatus BlockReader::init(const ReaderParams& read_params) {
    ...
    _origin_return_columns = read_params.origin_return_columns;
    _return_column_size = _origin_return_columns->size() - (_sequence_col_idx != -1 ? 1 : 0);
...
OLAPStatus BlockReader::_unique_key_next_block(Block* block, MemPool* mem_pool,
    ...
    do {
        ...
        auto cmp_res = _unique_key_tmp_block->compare_at(0, _next_row.second, tablet()->num_key_columns(),
                *_next_row.first, -1);
        if (cmp_res == 0) merged_count++;
        else {
            _insert_tmp_block_to(target_columns);
            target_block_row++;
            _replace_data_in_column();
...
void BlockReader::_insert_tmp_block_to(doris::vectorized::MutableColumns& columns) {
    const auto& return_column = (*_origin_return_columns);
    for (int i = 0; i < _return_column_size; i++) {
        columns[i]->insert_from(*_unique_row_columns[return_column[i]], 0);
    }
}
